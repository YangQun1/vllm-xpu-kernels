...
@torch.inference_mode()
def test_gdn_attention_head_dim_256():
    # Focused large-head-dim case: head_k_dim == head_v_dim == 256.
    # Simplified single combination: has_bias=False, bf16, prefill,
    # ssm_state_is_fp32=True, num_actual_tokens=8192.
    if (os.getenv("SKIP_ACC_ERROR_KERNEL") is not None
            and os.getenv("SKIP_ACC_ERROR_KERNEL") == "1"):
        pytest.skip("skip gdn attention kernels testing on PVC.")
    ...
    ssm_state_dtype = torch.float32  # ssm_state_is_fp32=True

    assert head_k_dim == head_v_dim

    num_prefills = batch_size  # prefill mode
    num_decodes = batch_size - num_prefills
    cache_batch_size = 200
