...
//   - Split-K fills GPU cores for all M values (20 XE-cores on BMG B60)
...
//
// Three regimes based on num_wg_m vs num_xe_cores:
//   num_wg_m ≤ num_xe_cores       → 32 splits (GPU severely underoccupied)
//   num_wg_m ≤ 4×num_xe_cores     → 4 splits  (moderate occupancy)
//   num_wg_m > 4×num_xe_cores     → 8 splits   (well-occupied, cache benefit)
// ===========================================================================
static int choose_n_splits(int M, int BLK_M, int num_xe_cores = 20) {
    int num_wg_m = (M + BLK_M - 1) / BLK_M;

    if (num_wg_m <= num_xe_cores)
        return 32;
    else if (num_wg_m <= 4 * num_xe_cores)
        return 4;
    else
        return 8;
}
                      ...
                      int              M)
{
    ...
    CUTE_UNROLL
    for (int i = 0; i < prefetch_dist; i++, k_tile_prefetch++) {
    ...
    float hc_post_mult_value, int sinkhorn_repeat)
{
    ...
    constexpr int HC3        = HC * (2 + HC);   // 24
    constexpr int HC3_PLUS1  = HC3 + 1;         // 25 (24 mixes + 1 sqrsum)
    constexpr int COMB_LANES = HC * HC;          // 16
