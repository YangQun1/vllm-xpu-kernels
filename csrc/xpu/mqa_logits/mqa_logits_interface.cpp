...
void check_fp8_paged_mqa_inputs(
  ...
  TORCH_CHECK(weights.dim() == 2, "weights must be [batch_size * next_n, heads]");
  ...
  TORCH_CHECK(weights.size(0) == batch_size * next_n, "weights first dim mismatch");
  TORCH_CHECK(weights.size(1) == heads, "weights heads mismatch");
  TORCH_CHECK(context_lens.size(0) == batch_size, "context_lens shape mismatch");
...
torch::Tensor fp8_mqa_logits(
    const torch::Tensor& q, // (seq_len, num_heads, head_dim)
    const torch::Tensor& kv, // (seq_len_kv, head_dim)
    const torch::Tensor& kv_scales, // (seq_len_kv)
    const torch::Tensor& weights, // (seq_len, num_heads)
    const torch::Tensor& cu_seqlen_ks, // (seq_len)
    const torch::Tensor& cu_seqlen_ke // (seq_len)
) { // (seq_len, seq_len_kv)
  ...
  if (vllm::xpu::is_xe2_arch()) {
    ...
    return fp8_mqa_logits_xe2(
        ...
        seq_len_kv);
#else
    TORCH_CHECK(false, "XE2 mqa logits kernel is not enabled in this build.");
#endif
  }
...
torch::Tensor fp8_paged_mqa_logits(
    const torch::Tensor& q_fp8, // (batch_size, next_n, heads, index_dim)
    const torch::Tensor& kv_cache_fp8, // (num_blocks, block_size, 1, index_dim+4) packed kv cache
    const torch::Tensor& weights, // (batch_size * next_n, heads)
    const torch::Tensor& context_lens, // (batch_size)
    const torch::Tensor& block_tables, // (batch_size, max_blocks)
  ...
  const int64_t max_blocks = block_tables.size(1);
    ...
    // Packed layout is [all values bytes][all scale bytes] per block, not
    // interleaved per slot. We must flatten per block first, then split.
    auto kv_cache_flat =
  ...
  if (vllm::xpu::is_xe2_arch()) {
    ...
    return fp8_paged_mqa_logits_xe2(
        q_fp8,
        kv_fp8, // not contiguous
        kv_scales_f32, // not contiguous
        weights,
        context_lens,
        block_tables,
        batch_size,
    ...
    TORCH_CHECK(
        false,
        "XE2 paged mqa logits kernel is not enabled in this build.");
#endif
  }

  TORCH_CHECK(
      false,
      "Only XE2 paged mqa logits kernel is supported currently.");
}
